...
class MemoryProfiler
  DEFAULTS = {:delay => 10, :string_debug => false}
	...
	def self.dump_all(aClassTallyHash,aFilename)
		File.open(aFilename,'w') do |file|
			aClassTallyHash.sort_by { |k,v| -v.abs }.each {|a,b| file.puts "#{a},#{b}"}
		end
	end
...
#           file.puts "Top 20"
#           delta.sort_by { |k,v| -v.abs }[0..100].sort_by { |k,v| -v}.each do |k,v|
#             file.printf "%+5d: %s (%d)\n", v, k.name, curr[k] unless v == 0
#           end
#           file.flush
					
					dump_all(curr,Time.now.strftime('log/memory_profiler-%y%m%d-%H%M%S.log'))
